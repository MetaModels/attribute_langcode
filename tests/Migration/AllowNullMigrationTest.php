...
namespace MetaModels\AttributeLangCodeBundle\Test\Migration;
...
/**
 * @covers \MetaModels\AttributeLangCodeBundle\Migration\AllowNullMigration
 */
class AllowNullMigrationTest extends TestCase
{
    public function testName(): void
    {
        ...
        self::assertSame('Allow null values in MetaModels "langcode" attributes.', $migration->getName());
    }
    ...
    public function testRun(object $configuration): void
    {
        ...
        if ($configuration->requiredTablesExist && $configuration->attributeConfigured) {
            ...
            $table1Columns = [
                'normal' =>
                    (new Column('normal', new StringType()))->setLength(5)->setDefault($columnDefault),
                'camelcase' =>
                    (new Column('camelCase', new StringType()))->setLength(5)->setDefault($columnDefault)
            ];
            $table2Columns = [
                'normal' =>
                    (new Column('normal', new StringType()))->setLength(5)->setDefault($columnDefault),
